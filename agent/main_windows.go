...
import (
	...
	"github.com/Ecotrace/agent/adapter/windows"
...
func main() {
	...
	deviceClass := flag.String("class", envOr("DEVICE_CLASS", "windows"), "Device class")
	pollInterval := flag.Duration("notify-interval", 30*time.Second, "Notification poll interval")
	flag.Parse()
	...
	log.Printf("[agent] starting EcoTrace Windows agent host=%s class=%s server=%s", hostname, *deviceClass, *baseURL)
	...
	// ── KeyStore ─────────────────────────────────────────────────────────────
	ks, err := windows.NewWindowsKeyStore()
	if err != nil {
	...
	// ── Registration (with notification consent) ─────────────────────────────
	regConfig := core.RegistrationConfig{
		...
		Hostname:                  hostname,
		OS:                        "windows",
		NotificationConsent:       true, // Windows agent opts into control-plane notifications
		NotificationChannel:       "balloon",
		NotificationPollIntervalS: int(pollInterval.Seconds()),
	...
	// ── Transport ─────────────────────────────────────────────────────────────
	transport := windows.NewWindowsTransport(*baseURL, httpClient, tlm.GetToken, tlm.On401)
	...
	// ── Metric Source ─────────────────────────────────────────────────────────
	metricSrc := windows.NewWindowsMetricSource()

	// ── Send Orchestrator ─────────────────────────────────────────────────────
	orchestrator := core.NewSendOrchestrator(buffer, transport)
	orchestrator.Start()

	// ── Notification inbox (Windows-only feature) ─────────────────────────────
	inbox := windows.NewWindowsNotificationInbox(*baseURL, httpClient, tlm.GetToken, tlm.On401)
	poller := core.NewNotificationPoller(inbox, *pollInterval)
	poller.Start()
