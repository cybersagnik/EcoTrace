...
export function CarbonByDeviceChart({ rows }: { rows: TelemetryRow[] }) {
  ...
  const data = useMemo(() => {
    ...
    rows.forEach((r) => map.set(r.deviceId || "unknown", (map.get(r.deviceId) ?? 0) + r.carbonG));
    return Array.from(map.entries())
      .map(([deviceId, carbonG]) => ({ deviceId, carbonG: Number(carbonG.toFixed(1)) }))
